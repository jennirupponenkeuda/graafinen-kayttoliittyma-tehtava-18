...
namespace graafinentehtava18
{
    ...
    {
        DataTable oppilaitos = new DataTable(); //Uusi tietotaulu
        DataTable vastuuHenkilot = new DataTable();
        DataTable yhteys = new DataTable(); // aletaan rakentamaan yhteyttä taulukoiden välille
        ...
        {
            string viite = oppilaitos.Rows[OppilaitosCB.SelectedIndex]["OID"].ToString();//Taulukoiden yhdistäminen, jotta saadaan talteen
            ...
            yhteys = vastuuHenkilot.Select("OID = " + viite).CopyToDataTable();
                                                                                    //Tässä tehdään viite, eli tehdään uusi taulukko, joka toimii yhteystaulukkona

            VastuuHenkilotCB.DataSource = yhteys; //VastuuHenkiloCB on yhteystaulukko
            VastuuHenkilotCB.DisplayMember = "VNimi"; //Näytetään vastuuhenkilön nimi
        ...
        private void TaytaOppilaitosTaulukko() //Tässä luodaan oppilaitostaulukko ja siihen sisältö
        {
        ...
        private void VastuuHenkilotCB_TextChanged(object sender, EventArgs e)
        {
            ...
            OsastoLB.Text = yhteys.Rows[VastuuHenkilotCB.SelectedIndex]["VOsasto"].ToString();
            EmailLB.Text = yhteys.Rows[VastuuHenkilotCB.SelectedIndex]["VSahkoposti"].ToString();
            PuhLB.Text = yhteys.Rows[VastuuHenkilotCB.SelectedIndex]["VPuhelin"].ToString();
        }
    }
    }
